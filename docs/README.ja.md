...
起動時、アプリは EVE の設定フォルダーを自動的に検索します。見つからない場合や別の場所を指定したい場合は、**設定フォルダーを手動選択**ボタンをクリックしてください。
...
| Linux | Wine / Proton のプレフィックスによって異なります |

**自動検索できない主な原因：**

- EVE が標準以外のディレクトリにインストールされている
- Linux/macOS でカスタムランチャー、Wine、または Proton を使用している
- 複数の EVE インストールや地域クライアント（セレニティ / インフィニティ）が存在する

**フォルダーの構造：**

フォルダーにはサーバー名のサブフォルダー（例：`c_tranquility`、`_tq_tranquility`、`c_serenity`）が 1 つ以上含まれている必要があります。各サーバーフォルダー内にはプロファイルフォルダー（`settings_Default`、`settings_Custom` など）があり、そこに `.dat` 設定ファイルが格納されています。
...
**ヒント：** 最上位のゲームフォルダー（例：`CCP/EVE`）を選択するか、サーバーのサブフォルダー（例：`c_tranquility`）を直接指定しても構いません — アプリが自動的に判別します。
